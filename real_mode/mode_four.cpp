...
File : mode_four.cpp
Function : Subscriber
Description : 미세먼지 수치에 따른 자율주행 모드
Author : SAP Possible
...
// 입력메시지로는 sap_mode_four패키지의 mode_msg메시지를 받도록 되어있다
...
// 메시지콜백함수로써, 밑에서설정 msg라는 이름의 토픽
// 메시지를 수신하였을때 동작하는 함수이다
// 입력메시지로는 sap_mode_four패키지의 Msg메시지를 받도록 되어있다
void Subscriber::msgCallback(const sap_mode_four::Msg::ConstPtr &msg) { // 라즈베리파이로부터 미세먼지 데이터를 받는다
	...
	while (true) {
		...
		if (mode_data == 4) {
			...
			if (goalReached) { // 목적지에 성공적으로 도착했을 경우 
				...
				ros::spinOnce();
				mode_data = mode.getMode(); // 현재 선택된 모드를 mode_data에 저장
				ROS_INFO("check check check mode= %d", mode_data); // 현재 선택된 모드를 표시한다


				if (mode_data == 0){
					cout<<"azsssss"<<endl;
					goalReached=navigation.moveToGoal(last_spotx,last_spoty); // 충전기 스팟으로 이동		
				}
			}	
