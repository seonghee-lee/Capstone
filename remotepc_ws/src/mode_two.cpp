...
public:
    ...
    ~Subscriber();
    void msgCallback(const sap_mode_one::Msg::ConstPtr& msg);
    int getMsg();
};
//Constructor
...
void Subscriber::msgCallback(const sap_mode_one::Msg::ConstPtr &msg) {		//라즈베리파이로부터 미세먼지 데이터를 받는다.
